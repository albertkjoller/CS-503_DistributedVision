...
def create_agent(env, **kwargs):
    ...
    return PPO(
        ...
        policy_kwargs={'features_extractor_class': ResNet18},
        verbose=1,
        seed=0,
        **kwargs
    )
...
def run(
    ...
    evaluation_callback = callbacks.EvalCallback(
        ...
        eval_freq=eval_freq,
        log_path='logs/evaluations/ppo_baseline_resnet',
        best_model_save_path='logs/models/ppo_baseline_resnet'
    )

    # Play!
    model = agent.learn(
        total_timesteps=total_timesteps,
        tb_log_name='ppo_baseline_resnet',
        callback=evaluation_callback
    )
    model.save('logs/models/ppo_baseline_resnet_final')
